...
import {assignAlisleAsync} from '../../../redux/Slice/aisleSlice'
...
const ScanAssignQr: React.FC<CamProps> = ({ navigation,route }) => {
      ...
      try {
        ...
        for (const scannedBarcode of barcodes) {
          if (scannedBarcode.rawValue && scannedBarcode.rawValue !== '') {
            ...
            dispatch(assignAlisleAsync(datapassing)).then((res:any)=>{
              console.log('Axios Response:', res);
              if(res.payload.status){
                // Alert.alert("barcode assign")
                Toast.show({
                  type: ALERT_TYPE.SUCCESS,
                  title: "Success",
                  textBody: 'Successfully barcode scan',
                })
              navigation.navigate('AssignAisleCamera',{
                data: datascan,
                aisleCode: a,
                shelfCode: b,
              });
  ...
  return (
    ...
    hasPermission && (
      <>
        ...
        <Text style={{ color: "white", fontWeight: '600',  fontSize: 22, zIndex: 2000 }}>Scan QR Code</Text>
        <View >
