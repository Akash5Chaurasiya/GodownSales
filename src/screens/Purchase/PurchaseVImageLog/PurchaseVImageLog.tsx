...
const PurchaseVImageLog = ({ navigation, route }: any) => {
  const dispatch = useDispatch();
  const { dataState } = route.params || {};
  const dataStatef = dataState
  console.log("DDDDDDDDDDDDDdc", dataState, dataStatef)
  ...
  const ImageAray: any = PurchaseData?.imageLogs

  console.log("data after scan ", PurchaseData)
  ...
  return (
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <View style={{ margin: '3%', borderRadius: 10, borderWidth: 0.1, flexDirection: 'row', alignItems: 'center' }}>
        ...
        />
        <Text className='text-center text-xl font-bold m-3 text-black underline'>Tap Aisle for verification:</Text>
            ...
            <TouchableOpacity style={[
              ...
              onPress={() => {
                if (!item.verify) {
                  ...
                  navigation.navigate('PurchaseVerification', { aisleID: item.aisle._id, dataState: dataState });
                } else {
...
export default PurchaseVImageLog
