import { StyleSheet, Text, View , TouchableOpacity,RefreshControl, FlatList,TextInput, SafeAreaView } from 'react-native'
...
const DTranfer = ({navigation}:any) => {
  ...
  const tranfer = useSelector((state:any)=> state.transfer.destTransfer)
  console.log("-----------------------", tranfer)
  //pending tranfer
  const tranferList = tranfer?.filter((tranfer:any)=>tranfer.status === 'Processing')
    ...
    <View style={{flex:1, backgroundColor:'white'}}>
       ...
       <Text className='text-center text-xl font-extrabold m-5 text-black'> Choose Voucher list</Text>
        <View style={{ marginHorizontal: '4%', flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center' }}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate("DTransferScanQr" )}>
            <Feather
              name="camera"
              size={25}
              color={'black'}
